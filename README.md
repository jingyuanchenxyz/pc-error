# Lean 4 proof of *Principal component error in high-dimensional factor models*

Machine-checked proof of every numbered result of Bernstein, Goldberg, Gunther,
Kercheval, Lan, Lin and Yao, *Principal component error in high-dimensional factor models*
(version of September 14, 2026). https://arxiv.org/abs/2609.20550 Lean 4.28.0, Mathlib v4.28.0.

Proved with the assistant of a Local Qwen3-Coder-480B-A35B-Instruct + Claude.

> **Formalized: 1–6 of the paper imply its Lemmas 1–6, Propositions 1–3,
> Corollaries 1–3 and Theorems 1–3.**

Building on the Ono / Axiom Math Proof, we have the three background facts the proofs use (Kolmogorov's strong law for non-identically distributed summands, Weyl's inequality, eigenvector continuity)  which are absent from Mathlib, so they are treated as explicit hypotheses of the statements, and all three are proved in `LatentError/Facts/`. `Facts/Unconditional.lean` restates the headline results with them discharged.
...
## Licence
We make use of:
https://github.com/AxiomMath/LatentError `LatentError/Ono/Solution.lean` is MIT, © 2026 Axiom Math, see `LatentError/Ono/LICENSE`.
